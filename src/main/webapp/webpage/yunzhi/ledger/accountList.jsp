...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<%--<button onclick="add('录入','accountController.do?goAdd','accountList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','accountController.do?goUpdate','accountList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
			...
			<button onclick="ExportXls('导出','','accountList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','accountList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#accountList').on(
					...
					function() {
						$('#btn_delete').prop('disabled',!$('#accountList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#accountList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#accountList').bootstrapTable({
									url : 'ledgerController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#accountList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#accountList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													/*href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('accountController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,user,client,balance,hotelInfo'
				};
	...
	<script type="text/javascript">
		...
		function listDictFormat(value,code,table){
			...
			var dicts = accountListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = accountListdictsData[dictKey];

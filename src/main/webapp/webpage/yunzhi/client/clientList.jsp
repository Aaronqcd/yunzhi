...
<body>
							...
							<form id="searchForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
											<div class="col-xs-12 col-sm-6 col-md-4">
												...
												<div class="input-group" style="width: 100%">
													<input type="text" class="form-control input-sm" id="hotelInfo" name="hotelInfo"/>
												</div>
											</div>
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','clientController.do?goAdd','clientList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','clientController.do?goUpdate','clientList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','clientController.do?doBatchDel','clientList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','clientController.do?goUpdate','clientList',600,400)" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<%--<button onclick="ImportXls('导入','','clientList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button>
			<button onclick="ExportXls('导出','','clientList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','clientList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#clientList').bootstrapTable({
									url : 'clientController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,hotelInfo,corpInfo,license,address,state,contact,'
				};
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'clientController.do?upload',"clientList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("clientController.do?exportXls","clientList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("clientController.do?exportXlsByT","clientList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = clientListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = clientListdictsData[dictKey];

...
<body>
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		...
		<div class="table-responsive">
			...
			<table id="deductionDetailList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		var deductionDetailListdictsData = {};
		$(function() {
			var promiseArr = [];
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(deductionDetailListdictsData,"deType",promiseArr,"","");
			}));
			...
			$('#deductionDetailList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#deductionDetailList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#deductionDetailList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#deductionDetailList').bootstrapTable({
									url : 'deductionDetailController.do?newDatagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#deductionDetailList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#deductionDetailList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												switchable : true,
												formatter : function(value,row, index) {
													return listDictFormat(value,"deType","");
												}
											},
											{
												field : 'status',
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,accountId,money,type,status,comment,beforeMoney,afterMoney'
				};
		...
		function reloadTable() {
			$('#deductionDetailList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'deductionDetailController.do?upload',"deductionDetailList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("deductionDetailController.do?exportXls","deductionDetailList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("deductionDetailController.do?exportXlsByT","deductionDetailList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = deductionDetailListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = deductionDetailListdictsData[dictKey];

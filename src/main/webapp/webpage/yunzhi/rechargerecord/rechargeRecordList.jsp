...
<body>
							...
							<form id="searchForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
											<div class="col-xs-12 col-sm-6 col-md-4">
												...
												<div class="input-group" style="width: 100%">
													<input type="text" class="form-control input-sm" id="money" name="money"/>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-md-4">
												<label for="prepaid">充值方式：</label>
												<div class="input-group" style="width: 100%">
													<select class="form-control input-sm" id="prepaid" name="prepaid"></select>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-md-4">
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','rechargeRecordController.do?goAdd','rechargeRecordList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 充值
			</button>
			<%--<button onclick="add('录入','rechargeRecordController.do?goAdd','rechargeRecordList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>--%>
			<%--<button onclick="update('编辑','rechargeRecordController.do?goUpdate','rechargeRecordList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','rechargeRecordController.do?doBatchDel','rechargeRecordList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>--%>
			<button onclick="detail('查看','rechargeRecordController.do?goUpdate','rechargeRecordList',600,400)" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="rechargeRecordList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		var rechargeRecordListdictsData = {};
		$(function() {
			var promiseArr = [];
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(rechargeRecordListdictsData,"prepaid",promiseArr,"","");
			}));
			
			Promise.all(promiseArr).then(function(results) {
				loadSearchFormDicts($("#searchForm").find("select[name='prepaid']"),"","prepaid","select","充值方式");
			...
			$('#rechargeRecordList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#rechargeRecordList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#rechargeRecordList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#rechargeRecordList').bootstrapTable({
									url : 'rechargeRecordController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#rechargeRecordList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#rechargeRecordList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												switchable : true,
												formatter : function(value, rec, index) {
													return listDictFormat(value,"prepaid","");
												}
											},
											{
												field : 'status',
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													/*href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('rechargeRecordController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,accountId,money,prepaid,comment,status,beforeMoney,afterMoney'
				};
		...
		function reloadTable() {
			$('#rechargeRecordList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'rechargeRecordController.do?upload',"rechargeRecordList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("rechargeRecordController.do?exportXls","rechargeRecordList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("rechargeRecordController.do?exportXlsByT","rechargeRecordList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = rechargeRecordListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = rechargeRecordListdictsData[dictKey];

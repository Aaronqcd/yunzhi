...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','staffAccountController.do?goAdd','staffAccountList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','staffAccountController.do?goUpdate','staffAccountList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="detail('查看','staffAccountController.do?goUpdate','staffAccountList',600,400)" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="staffAccountList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#staffAccountList').on(
					...
					function() {
						$('#btn_delete').prop('disabled',!$('#staffAccountList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#staffAccountList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#staffAccountList').bootstrapTable({
									url : 'staffAccountController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#staffAccountList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#staffAccountList').bootstrapTable('getOptions').pageNumber;
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,userId,cardNo,accountName,bankName,balance,type,status,'
				};
		...
		function reloadTable() {
			$('#staffAccountList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'staffAccountController.do?upload',"staffAccountList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("staffAccountController.do?exportXls","staffAccountList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("staffAccountController.do?exportXlsByT","staffAccountList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = staffAccountListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = staffAccountListdictsData[dictKey];

...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="generateInviteCode()" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 生成邀请码
			</button>
			<button onclick="detail('查看','inviteCodeController.do?goUpdate','inviteCodeList',600,400)" class="btn btn-info btn-sm">
	...
	<script type="text/javascript">
		...
		function generateInviteCode() {
			$.ajax({
				type: "post",
				url: "inviteCodeController.do?generateInviteCode",
				dataType:"json",
		...
		$(function() {
			...
			$('#inviteCodeList').on(
					...
					function() {
						$('#btn_delete').prop('disabled',!$('#inviteCodeList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#inviteCodeList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#inviteCodeList').bootstrapTable({
									url : 'inviteCodeController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#inviteCodeList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#inviteCodeList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('inviteCodeController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,inviteCode,userId,useDate,status,'
				};
		...
		function reloadTable() {
			$('#inviteCodeList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'inviteCodeController.do?upload',"inviteCodeList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("inviteCodeController.do?exportXls","inviteCodeList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("inviteCodeController.do?exportXlsByT","inviteCodeList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = inviteCodeListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = inviteCodeListdictsData[dictKey];
